...
func NewRunCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			runEnv.Task, err = runEnv.TaskRepo.GetTaskById(ctx, runEnv.TaskId)
			if err != nil {
				if err == database.ErrDoesNotExist {
					return fmt.Errorf("task %d does not exist, for more info see 'glesha help add'", runEnv.TaskId)
				}
				return err
			}
			L.Printf("%s", runEnv.Task)
			err = config.Parse(runEnv.Task.ConfigPath)
			if err != nil {
				return err
			}

			if config.Get().Autogenerated {
				L.Warn(config.GetAutoGenConfigWarning(runEnv.Task.ConfigPath))
			}
			return runTask(ctx, runEnv)
		},
	}
